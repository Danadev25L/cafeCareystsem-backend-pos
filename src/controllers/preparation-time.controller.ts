...
export const startPreparation = async (
  ...
  try {
    ...
    const existingPrepTime = await prisma.preparationTime.findFirst({
      where: {
...
export const completePreparation = async (
  ...
  try {
    ...
    const preparationTime = await prisma.preparationTime.findFirst({
      where: {
        orderId: parseInt(orderId),
        status: 'PREPARING',
      },
    ...
    const updatedPrepTime = await prisma.preparationTime.update({
...
export const getKitchenQueue = async (
  ...
  try {
    ...
    const ordersWithPrepTime = orders.map((order: { preparationTime: Array<{ startTime: Date | null }> }) => {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        queueStats: {
          ...
          rushOrders: orders.filter((o: { isRush: boolean }) => o.isRush).length,
          highPriorityOrders: orders.filter((o: { priority: string }) => o.priority === 'HIGH' || o.priority === 'URGENT').length,
...
export const getPreparationStats = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    const prepTimes = await prisma.preparationTime.findMany({
      where: {
        status: 'COMPLETED',
        endTime: {
          gte: startDate,
    ...
    const averagePrepTime = totalPreparations > 0
      ? prepTimes.reduce((sum: number, prep: { duration: number | null }) => sum + (prep.duration || 0), 0) / totalPreparations
      : 0;

    // Preparation time by item count
    const prepTimeByItemCount: { [key: number]: number[] } = {};
    prepTimes.forEach((prep: { order: { items: unknown[] }; duration: number | null }) => {
    ...
    // Daily trends
    const dailyStats: { [key: string]: { count: number; averageTime: number; totalTime: number } } = {};
    prepTimes.forEach((prep: { order: { items: unknown[] }; duration: number | null; endTime: Date | null }) => {
    ...
    const finalDailyStats: { [key: string]: { count: number; averageTime: number } } = {};
...
// Helper functions
function estimatePrepTime(order: any): number {
  // Base preparation time per item
  const baseTimePerItem = 3; // 3 minutes per item
  const itemCount = order.items.length;

  // Complexity factor based on item types
  const complexityFactor = order.items.some((item: any) =>
    item.menuItem.nameEn?.toLowerCase().includes('coffee') ||
    item.menuItem.nameEn?.toLowerCase().includes('espresso')
  ) ? 1.5 : 1.0;
...
function getPriorityLevel(order: any): number {
  if (order.isRush) return 100;
  if (order.priority === 'URGENT') return 90;
  if (order.priority === 'HIGH') return 80;
  if (order.priority === 'LOW') return 20;
  return 50; // NORMAL
}

function calculateAveragePrepTime(orders: any[]): number {
  const completedPreps = orders.filter(o =>
    o.preparationTime.length > 0 &&
    o.preparationTime[0].duration
  );

  if (completedPreps.length === 0) return 0;

  const totalTime = completedPreps.reduce((sum, order) =>
    sum + order.preparationTime[0].duration!, 0
  );

  return Math.round(totalTime / completedPreps.length);
}

function calculateEfficiency(prepTimes: any[]): number {
  ...
  const efficiencyScores = prepTimes.map(prep => {
    ...
    if (ratio <= 1.2 && ratio >= 0.8) return 100; // Within 20% of estimate
    if (ratio <= 1.5 && ratio >= 0.5) return 80;  // Within 50% of estimate
