...
async function runCommand(command: string, description: string, allowFailure = false): Promise<boolean> {
  try {
...
async function resetAndDeploy() {
  try {
    ...
    try {
      ...
      execSync('node dist/scripts/drop-all-tables.js', {
        stdio: 'inherit',
        env: process.env,
      });
    } catch (error: any) {
    ...
    try {
      ...
      try {
        ...
        migrationsSuccess = await runCommand(
    ...
    try {
      execSync('node dist/scripts/verify-tables.js', {
        stdio: 'inherit',
        env: process.env,
      });
    } catch (error: any) {
